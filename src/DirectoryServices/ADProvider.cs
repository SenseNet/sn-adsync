...
namespace SenseNet.DirectoryServices
{
    public class ADProvider : DirectoryProvider
    {
        ...
        public static void RetryAllFailedActions()
        {
            using (new SystemAccount())
            {
                var qs = new QuerySettings { EnableLifespanFilter = FilterStatus.Disabled, EnableAutofilters = FilterStatus.Disabled };
        ...
        {
            return FormsAuthenticationFromADSection.Current.AuthSettings.Cast<AuthSettingElement>()
                .FirstOrDefault(authSetting => string.Equals(authSetting.Domain, domain, StringComparison.InvariantCultureIgnoreCase));
        }
        ...
        public override bool IsADAuthenticated(string domain, string username, string pwd)
        {
            ...
            {
                return Common.IsADCustomAuthenticated(authSetting.ADServer, username, pwd, authSetting.CustomLoginProperty, authSetting.CustomADAdminAccountName, authSetting.CustomADAdminAccountPwd);
            }

            return Common.IsADAuthenticated(authSetting.ADServer, domain, username, pwd, AuthSettingElement.LoginProperty);
        }

        public override bool SyncVirtualUserFromAD(string domain, string username, User virtualUser)
        {
            throw new NotImplementedException("This feature is obsolete, because it uses the deprecated AD sync configuration.");

            //TODO: implement this based on the new AD to Portal sync setting, instead of using the old config
        ...
        public override bool SyncVirtualUserFromAD(Guid guid, User virtualUser)
        {
            ...
            //var formsADAuthSettings = FormsAuthenticationFromADSection.Current.AuthSettings;
